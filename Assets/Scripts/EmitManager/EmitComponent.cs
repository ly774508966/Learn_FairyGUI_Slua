...
public class EmitComponent : GComponent {
    ...
    public void SetHurt(Transform owner, int type, long hurt, bool critical)
    {
        ...
        if (type == 0)
        {
            tf.font = EmitManager.inst.hurtFontYellow;
        }
        else
        {
            tf.font = EmitManager.inst.hurtFontRed;
        }
    ...
    {
        Vector3 ownerPos = _owner.position;  // 获取世界坐标
        ownerPos.y += OFFSET_ADDITION;
        Vector3 screenPos = Camera.main.WorldToScreenPoint(ownerPos);
        screenPos.y = Screen.height - screenPos.y;  //转换为屏幕坐标

        Vector3 pt = GRoot.inst.GlobalToLocal(screenPos);
        this.SetXY(Mathf.RoundToInt(pt.x + pos.y - this.actualWidth / 2), Mathf.RoundToInt(pt.y + pos.y - this.height));
